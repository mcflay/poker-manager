/**
 * Seed script: imports the latest WPT scraper JSON into the database.
 * Usage: node scripts/seed-wpt.mjs [path-to-json]
 */
...
function detectSpeed(name) {
  ...
  if (lower.includes("deepstack") || lower.includes("deep stack")) return "deep";
  return "regular";
}
...
function detectFormat(name, type) {
  ...
  if (lower.includes("sng")) return "SNG";
  return "MTT";
}
...
function normalizeGameType(gt, name) {
  ...
  if (lower.includes("plo") || lower.includes("omaha")) return "PLO";
...
if (!jsonPath) {
  // Auto-find latest in ../Scrapers/output
...
if (!jsonPath || !existsSync(jsonPath)) {
  console.error("No JSON file found. Run the WPT scraper first or pass path as argument.");
  process.exit(1);
}
...
// Ensure tables exist (same DDL as in db/index.ts)
db.exec(`
...
const importAll = db.transaction((items) => {
  for (const t of items) {
    if (!t.start_time || !t.name) { skipped++; continue; }

    const stableId = `wptglobal_${Buffer.from(t.name + t.start_time).toString("base64url").slice(0, 32)}`;
